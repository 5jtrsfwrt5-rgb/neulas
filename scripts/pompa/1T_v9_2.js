// ================================================
// SCRIPT 1T — Trasmittente (v9.2)
// Shelly 1 PM Gen4 + LoRa Add-on — FW >= 2.0.0
// Novità v9.2:
//  - Temperatura di 1R nei PONG:
//    "PONG:OFF:<temp>" e "PONG:ON:<residuo>:<watt>:<temp>"
//    (log dedicato + allarme opzionale CFG_TEMP_MAX)
//  - Verifica ESITO CICLO dopo perdita di comunicazione:
//    * PONG:ON dopo allarme -> "comunicazione ripristinata,
//      pompa ancora in funzione (residuo Xs)"
//    * PONG:OFF a tempo previsto trascorso -> "ciclo COMPLETATO
//      (verificato via PONG)" anche se POMPA_OFF era andato perso
//    * PONG:OFF prima del termine -> "ciclo interrotto" (come prima,
//      ma ora con i secondi mancanti)
//  - fineCiclo riallineato ad ogni PONG:ON:<residuo>
//  - Ping di verifica anticipato alla scadenza del timer locale
//    se l'esito del ciclo non e' ancora confermato
//  - Fix: notifica "ciclo completato" su POMPA_OFF anche se il
//    timer locale era appena scaduto (corsa temporale)
// ================================================
...
// Allarme temperatura di 1R (0 = disattivato, es. 70 = avviso oltre 70 °C).
// Rientro con isteresi di 5 °C.
let CFG_TEMP_MAX            = 0;
...
let allarmeTempInviato    = false;  // v9.2: sovratemperatura 1R gia notificata
...
function impostaTimerLocale(secondi) {
  ...
  timerPompa = Timer.set(secondi * 1000, false, function() {
    ...
    // v9.2: esito non ancora confermato -> ping di verifica anticipato
...
// v9.2: verifica esito ciclo (completato / interrotto)
// ------------------------------------------------
function sincronizzaStato(statoOn, residuo) {
  ...
  if (statoOn) {
    ...
    // v9.2: 1R risponde di nuovo dopo l'allarme -> ciclo ancora in corso
    ...
    if (esitoCicloPendente) {
      // v9.2: pompa OFF con esito pendente -> completato o interrotto?
...
// TEMPERATURA 1R (v9.2)
// ------------------------------------------------
function gestisciTemperatura(tStr) {
  print("Temperatura 1R: " + tStr + " °C");
  if (CFG_TEMP_MAX <= 0) return;
  let t = parseInt(tStr);
  if (isNaN(t)) return;
  if (t >= CFG_TEMP_MAX && !allarmeTempInviato) {
    allarmeTempInviato = true;
    inviaNtfy("🌡⚠️ Temperatura di 1R elevata: " + tStr + " °C (soglia " + JSON.stringify(CFG_TEMP_MAX) + " °C).");
  } else if (allarmeTempInviato && t <= CFG_TEMP_MAX - 5) {
    allarmeTempInviato = false;
    if (CFG_NOTIFICA_RIPRISTINO) inviaNtfy("🌡 Temperatura di 1R rientrata: " + tStr + " °C.");
  }
}
...
Shelly.addEventHandler(function(event) {
  ...
  // Formati v9.2: "PONG:OFF[:<temp>]", "PONG:ON:<residuo>[:<watt>[:<temp>]]",
  ...
  if (msg.indexOf("PONG") === 0) {
    ...
    if (msg.indexOf("PONG:OFF") === 0) {
      if (msg.length > 9) tStr = msg.slice(9);   // "PONG:OFF:" = 9 caratteri
      ...
      if (msg.length > 8) {                      // "PONG:ON:" = 8 caratteri
        ...
        if (campi.length > 2) tStr = campi[2];
      }
      sincronizzaStato(true, residuo);
      controllaPotenza(watt);
    }

    if (tStr !== "") gestisciTemperatura(tStr);
  ...
  if (msg === "POMPA_ON") {
    ...
    if (ackAtteso) {
      ackAtteso      = false;
      contatoreRetry = 0;
      // v9.2: da ora l'esito del ciclo va tracciato fino a conferma
      esitoCicloPendente = true;
  ...
  if (msg === "POMPA_OFF") {
    ...
    ackSpegniAtteso      = false;
    contatoreRetrySpegni = 0;
    // v9.2: eraCiclo copre anche la corsa temporale in cui il timer
...
// === AVVIO ===
configuraSheLR();
print("Sistema avviato (v9.2, SheLR cifrato). Primo ping tra " + JSON.stringify(CFG_PING_AVVIO) + " secondi.");
