// ================================================
// SCRIPT 1R — Ricevente (v9.2)
// Shelly 1 PM Gen4 + LoRa Add-on — FW >= 2.0.0
// Novità v9.2:
//  - Temperatura di esercizio nel PONG:
//    "PONG:OFF:<temp>" e "PONG:ON:<residuo>:<watt>:<temp>"
//    (temperatura interna del dispositivo, un decimale;
//    la valutazione resta compito di 1T)
//  - Nessun altro cambiamento funzionale rispetto a v9.1
// ================================================
...
// ------------------------------------------------
// TEMPERATURA (v9.2): formatta tC con un decimale senza
// artefatti di conversione float->stringa
// ------------------------------------------------
...
Shelly.addEventHandler(function(event) {
  ...
  // ---- PING: rispondi con stato reale (+ potenza e temperatura) ----
  if (msg === "PING") {
    let sw = Shelly.getComponentStatus("switch:0");
    let t  = leggiTemp(sw);   // ":<temp>" oppure "" se non disponibile
    if (sw && sw.output === true && timerSpegnimento !== null) {
      ...
      loraSend("PONG:ON:" + JSON.stringify(residuo) + ":" + JSON.stringify(w) + t);
    } else {
      loraSend("PONG:OFF" + t);
    }
    return;
  ...
  if (msg.indexOf("ACCENDI_POMPA") === 0) {
    ...
    // IDEMPOTENTE: se la pompa e' gia accesa con timer attivo,
...
configuraSheLR();
print("Script 1R v9.2 attivo. In ascolto (SheLR)...");