// ================================================
// SCRIPT 1T — Trasmittente (v9.1)
// Shelly 1 PM Gen4 + LoRa Add-on — FW >= 2.0.0
// Novità v9.1:
//  - Ping accelerato SOLO durante la pompa accesa
//    (CFG_PING_POMPA) per rilevare in fretta black-out
//    o guasti entro la finestra dei 400 s
//  - Soglia allarme dedicata a pompa accesa
//    (CFG_MAX_ALERT_POMPA) con notifica specifica
//  - Notifica "ciclo interrotto" se 1R torna con
//    PONG:OFF mentre la pompa doveva essere accesa
//  - Controllo assorbimento pompa (CFG_POTENZA_MIN):
//    1R riferisce i watt nel PONG:ON:<res>:<watt>
//  - Anti-collisione radio: PING rinviato se c'e' uno
//    scambio ACCENDI/SPEGNI in corso; POMPA_ON/POMPA_OFF
//    contano come prova che 1R e' vivo (meno falsi allarmi)
// ================================================
...
// ------------------------------------------------
// PIANIFICAZIONE PING (v9.1: ritmo diverso a pompa accesa)
// ------------------------------------------------
...
function intervalloPingAlert() {
  // A pompa accesa l'alert non deve mai essere piu lento del ritmo pompa
  return (isPompaOn && CFG_PING_POMPA < CFG_PING_ALERT) ? CFG_PING_POMPA : CFG_PING_ALERT;
}

// ------------------------------------------------
// PROVA DI VITA (v9.1): qualsiasi messaggio valido da 1R
...
function sincronizzaStato(statoOn, residuo) {
  ...
  if (statoOn) {
    ...
    if (isPompaOn) {
      ...
      // v9.1: la pompa doveva essere accesa -> ciclo interrotto
      inviaNtfy("⚠️ Ciclo pompa interrotto prima del termine (probabile interruzione di corrente su 1R).");
    }
  }
}

// ------------------------------------------------
// CONTROLLO ASSORBIMENTO POMPA (v9.1)
// ------------------------------------------------
...
function inviaPing() {
  ...
  // v9.1: non contendere la radio a uno scambio comando in corso
  if (ackAtteso || ackSpegniAtteso) {
...
function verificaPong() {
  ...
  if (pongRicevuto) {
    ...
    // v9.1: avviso rapido dedicato quando la pompa dovrebbe essere accesa
...
Shelly.addEventHandler(function(event) {
  ...
  // ---- PONG (con stato reale e potenza) ----
  // Formati: "PONG:OFF", "PONG:ON:<residuo>", "PONG:ON:<residuo>:<watt>",
  ...
  if (msg.indexOf("PONG") === 0) {
    ...
    if (msg === "PONG:OFF") {
      ...
      if (msg.length > 8) {                    // "PONG:ON:" = 8 caratteri
        let resto = msg.slice(8);              // "<residuo>" o "<residuo>:<watt>"
        let sep = resto.indexOf(":");
        if (sep === -1) {
          let r = parseInt(resto);
          if (!isNaN(r) && r > 0) residuo = r;
        } else {
          let r = parseInt(resto.slice(0, sep));
          if (!isNaN(r) && r > 0) residuo = r;
          let w = parseInt(resto.slice(sep + 1));
  ...
  if (msg === "POMPA_ON") {
    ...
    if (ackAtteso) {
      ...
      // v9.1: passa subito al ritmo ping "pompa accesa"
  ...
  if (msg === "POMPA_OFF") {
    ...
    if (eraRichiesto) {
      ...
      if (CFG_NOTIFICA_SPEGNIMENTO && eraOn) inviaNtfy("✅ Elettropompa spenta: ciclo completato.");
    }
    return;
  }
...
// === AVVIO ===
configuraSheLR();
print("Sistema avviato (v9.1, SheLR cifrato). Primo ping tra " + JSON.stringify(CFG_PING_AVVIO) + " secondi.");
